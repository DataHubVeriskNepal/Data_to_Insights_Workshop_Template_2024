...
class CSVHandler:
    def __init__(self, folder, headers, date):
        # Convert the date string to a datetime object
        self.date = datetime.strptime(date, '%Y-%m-%d')
        self.folder = folder
        self.filename = f'{self.folder}_{self.date.strftime("%Y_%m")}.csv'
        ...
        try:
            ...
            if os.stat(self.filepath).st_size == 0:  # Write header if file is empty
                self.writer.writerow(headers)  # Header row
    ...
    def close(self, s3_uploader):
        if self.file is not None:
            try:
                ...
                s3_file = f'{self.folder}/{self.filename}'  # S3 key
